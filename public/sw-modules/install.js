					...
					fetch('/asset-manifest.json').then(response => {
						response.json().then(assets => {
							...
							* */
							...
							// 1) Cache static files (priority 0)
							// TODO: Remove comment from service-worker.js
							const urlsToCache = [
								'/',
								// '/service-worker.js',
								assets['main.js'],
							...
							console.log("INSTALL","ADDING STATIC");
							cache.addAll(urlsToCache);

							console.log("INSTALL","ADDING DUMMY");
							...
							// 2) Save current timestamp -> so we can update cache, when it is older then X
							console.log("INSTALL","ADDING TIMESTAMP");
							...
							console.log("INSTALL","ADDING API");
